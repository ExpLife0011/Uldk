	...
	grub_video_bitmap_h_align_t h_align)
{
	*dst = 0;
	...
	if (ret == GRUB_ERR_NONE)
	{
		...
		src->data = (UINT8 *)src->data + sx0 * bytes_per_pixel
			+ sy0 * sstride;
		(*dst)->mode_info.width = dw;
		(*dst)->mode_info.height = dh;
		(*dst)->data = (UINT8 *)(*dst)->data + dx0 * bytes_per_pixel
			+ dy0 * dstride;
...
scale_nn(struct grub_video_bitmap *dst, struct grub_video_bitmap *src)
{
	...
	UINT8 *ddata = dst->data;
	UINT8 *sdata = src->data;
